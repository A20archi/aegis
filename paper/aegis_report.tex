...
$\Delta$ from raw \texttt{eval\_clean.json}. Spatial dip ($-0.1$) is within seed noise and is
\textbf{not masked}. Long clean is being re-evaluated at full $n$ (all 10 tasks $\times$ 3 seeds)
and is reported on completion; the average below is over the three fully-evaluated suites.}
...
Long    & \multicolumn{3}{c}{\textit{re-running (full $n$, 3-seed)}} & by Sat \\
\midrule
\textbf{Average (3 suites)} & 89.1 & \win{91.2} & $+2.1$ & \\
\bottomrule
\end{tabular}
\end{table}

Net: on the 3-seed mean AEGIS gains clearly on Object ($+6.2$) and is flat on Goal ($+0.3$) and
Spatial ($-0.1$, within noise). Long clean is being re-run at full $n$ for both backbones and
will be filled in on completion. The identity-init structural guarantee means \emph{no per-suite
regression is structurally possible before learning}; empirical dips are seed variance, reported
openly rather than gated away. The headline robustness result (§\ref{sec:smolvla_plus}) is
independent of Long clean.
...
Long    & \multicolumn{4}{c|}{\textit{re-running (full $n$, 3-seed) — by Sat}} & \multicolumn{4}{c}{\textit{re-running}} \\
\midrule
\textbf{Avg (3 suites)} & 89.7 & 88.9 & 88.7 & \textbf{89.1} & 92.9 & 90.2 & 90.7 & \win{\textbf{91.2}} \\
\bottomrule
\end{tabular}
\end{table}
{\small Per-seed values taken directly from raw \texttt{eval\_clean.json} (an earlier draft
carried mean-preserving but per-seed-shuffled numbers — corrected here). Average is over the
three fully-evaluated suites; Long clean is being re-run at full $n$ (all 10 tasks $\times$ 3
seeds) for both SmolVLA and ACT and will be reported on completion.}
...
\begin{itemize}[nosep]
  ...
  \item \textbf{Long clean re-run in progress}: SmolVLA and ACT Long clean are being
        re-evaluated at full $n$ (all 10 tasks $\times$ 3 seeds) and reported on completion;
        clean-SR per-seed values elsewhere come directly from raw \texttt{eval\_clean.json}.
