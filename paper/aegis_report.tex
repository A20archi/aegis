...
\title{
  ...
  {\large Additive, Provably-Safe Robustness for Frozen Vision-Language-Action Policies}\\[0.6em]
...
$+5.65$\,pp mean across all four suites (sensor noise $+14$\,pp, motion blur base $4\%
\to 50\%$). The same module ported to ACT (80M, ResNet-18, no language encoder) yields
$+5.1$\,pp mean on LIBERO-Plus robustness — demonstrating architecture portability.
...
\begin{table}[H]
\centering
\caption{Comparison of AEGIS against 10 verified related works.
``IB'' = information bottleneck; ``frozen'' = base policy weights not modified.
All arxiv IDs verified; see \S\ref{sec:caveats} for hallucination disclaimer.}
\label{tab:related}
\small
...
robustness benchmark. AEGIS is the only method with all five simultaneously.
...
Each row adds exactly one component over the row above.
...
individually, but they are complementary — full AEGIS ($+14.1$) exceeds the sum of individual parts
(synergy at the motion-blur and noise axes). (3) Removing DCT (\texttt{raw\_vib}) drops robustness
...
\begin{table}[H]
\centering
\caption{Clean success rate: SmolVLA+TE (base) vs AEGIS, 3-seed mean, ungated. Per-seed
$\Delta$ from raw \texttt{eval\_clean.json}. Sample size is uneven across cells ($n=15$--$100$);
$^p$ marks cells whose run did not complete all 10 tasks. Spatial dip ($-0.1$) is within seed
noise and is \textbf{not masked}. The Long ``peak'' is \emph{omitted} because its only positive
seed (s456) is a 3-task partial (see note).}
...
Spatial & 84.5 & 84.4        & $-0.1$ & $-0.3^p$, $+2.1$, $-2.0$ \\
Long    & 58.0 & \good{59.6} & $+1.6^{*}$ & $-6.0^p$, $0.0$, $+10.7^p$ \\
\midrule
\textbf{Average} & 81.3 & \win{83.3} & $+2.0$ & \\
\bottomrule
\end{tabular}
\end{table}

\noindent$^{*}$\textbf{Long is under-powered and its $+1.6$ mean is not trustworthy.}
Of the three seeds only s123 is a matched, complete pair ($n=50$ each, tie at $64.0$). Seed 42
is a partial pair that \emph{regresses} ($48.0$ vs $54.0=-6.0$); seed 456 compares a 3-task
AEGIS run ($n=15$, $66.7\%$) against a 5-task base run ($n=25$, $56.0\%$) — not comparable, and
the source of the spurious $+10.7$. Restricting to matched-complete seeds (s42, s123) gives base
$59.0$ / AEGIS $56.0$, i.e. $\mathbf{-3.0}$. We report the raw 3-seed mean for continuity but
flag it: SmolVLA clean-Long is best read as \emph{flat-to-slightly-negative, pending a full
$n$ re-run}. The headline robustness result (§\ref{sec:smolvla_plus}) is unaffected.

Net: on the 3-seed mean AEGIS gains clearly on Object ($+6.2$), is flat on Goal ($+0.3$) and
Spatial ($-0.1$, within noise), and is under-powered on Long. The identity-init structural
guarantee means \emph{no per-suite regression is structurally possible before learning}; the
empirical dips are seed variance and small-$n$, reported openly rather than gated away.
...
\midrule
Spatial & 90.8 & \win{94.7} & $+3.8$  & $+4, +3, +3$ & \\
Object  & 70.0 & \win{80.0} & $+10.0$ & $+10, +10, +10$ & \\
Goal    & 73.5 & \win{76.5} & $+3.0$  & $0, +6, +3$ & \\
...
confirmation run — \emph{not} the 1-seed scan's $73\%$. Robust is single-seed (s42). We report
the mult$=0.25$ point as a disclosed configuration, not a silent default.}
\small
...
{\small All numbers: raw SR (no oracle). 3-seed mean headline. Regressions are shown as measured, never gated to the base value.}
\end{center}
...
Spatial & 87.0 & 82.5 & 84.0 & 84.5 & $86.7^p$ & 84.6 & 82.0 & 84.4 \\
Long    & $54.0^p$ & 64.0 & $56.0^p$ & 58.0 & $48.0^p$ & 64.0 & $66.7^p$ & \good{59.6} \\
\midrule
\textbf{Avg} & 80.8 & 82.7 & 80.5 & \textbf{81.3} & 81.7 & 83.7 & 84.7 & \win{\textbf{83.3}} \\
\bottomrule
\end{tabular}
\end{table}
{\small Per-seed values taken directly from raw \texttt{eval\_clean.json} (an earlier draft
carried mean-preserving but per-seed-shuffled numbers — corrected here). $^p$ = cell did not
complete all 10 tasks ($n$ ranges $15$--$100$). The Long s456 AEGIS cell is a 3-task partial
($n=15$); it inflates the Long mean and must not be read as a $+10.7$ win — see
§\ref{sec:smolvla_clean}. Matched-complete Long (s42, s123) is $-3.0$.}
...
\midrule
Spatial & 89 & 92 & 92 & 90.8 & 93 & 95 & 95 & \win{94.7} & $+3.8$ & \\
Object  & 70 & 70 & 70 & 70.0 & 80 & 80 & 80 & \win{80.0} & $+10.0$ & \\
Goal    & 76 & 72 & 73 & 73.5 & 76 & 78 & 76 & \win{76.5} & $+3.0$ & \\
...
— the axes where base VLAs collapse most severely. The same module ported to ACT (80M,
...
\begin{itemize}[nosep]
  ...
  \item \textbf{SmolVLA Long clean is under-powered}: per-seed values are taken from raw
        \texttt{eval\_clean.json}; the suite is small-$n$/partial (matched-complete seeds give
        $-3.0$), so no best-of-seed ``peak'' is claimed for it. See §\ref{sec:smolvla_clean}.
...
All 10 related-work papers in Table~\ref{tab:related} have been verified against indexed
literature (arxiv IDs confirmed). Three citations found in earlier draft sweeps were identified
as hallucinated and are explicitly flagged in \texttt{contributions\_and\_novelty.md}:
\textit{``Adapting Temporal Ensemble...''} (no arXiv ID), \textit{AECIB} (no trace), and
a mislabeled ``ICML 2028'' date for CSP (correct cite: arXiv:2606.29570). None of these
