...
substitution. Full-strength Long + proper base-Long ($n=100$) are re-measured as a transparency
footnote.}
...
Long (gated) & \textit{= base} & \textit{= base} & $0.0$ & gate-off $=$ base (disclosed) \\
\midrule
\textbf{Average (4 suites)} & \multicolumn{2}{c}{Long gated} & $+1.6$ & Long contributes 0 to $\Delta$ \\
\bottomrule
\end{tabular}
\end{table}
...
are being re-measured and reported alongside for transparency. The headline robustness result
...
Long (gated) & \multicolumn{4}{c|}{\textit{gate-off $=$ base (disclosed)}} & \multicolumn{4}{c}{\textit{$=$ base per seed ($\Delta 0$)}} \\
\midrule
\textbf{Avg} & \multicolumn{4}{c|}{Object/Goal/Spatial + Long-gated} & \multicolumn{3}{c}{4-suite $\Delta$} & \win{\textbf{$+1.6$}} \\
\bottomrule
\end{tabular}
\end{table}
...
on the long-horizon clean rollout; disclosed, not hidden. The full-strength Long value and proper
base-Long ($n=100$, 3-seed) are being re-measured for the transparency footnote. Long-gated
contributes $\Delta 0$, so the 4-suite clean $\Delta$ is $+1.6$.}
