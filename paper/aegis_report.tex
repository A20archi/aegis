...
\begin{tcolorbox}[colback=pendingcolor!5!white,colframe=pendingcolor!60!black,
  title={\textbf{AEGIS vs StableVLA-native — 3-arm head-to-head (base / StableVLA-native / AEGIS)}},breakable]
\textbf{[LIBERO-Object 3-seed done; Goal/Spatial/Long in progress.]}

\textbf{Setup:} VLA-Adapter-Pro (Prismatic Qwen2.5-0.5B + DINOSigLIP, frozen), the same base
architecture used by StableVLA. \textbf{AEGIS} ($\alpha=16$ vision-only LoRA r32 + RASF,
$\sim$24.8M) is compared head-to-head against \textbf{StableVLA-native}: the faithful FusedFAN
IB-Adapter (\texttt{nn\_utils.py} arch, verbatim), warm-started to a functional start and trained
with StableVLA's \emph{own} clean-task objective (clean-anchor BC). Under its native objective the
gate learns to go dormant, so the module becomes a pass-through and reproduces the base policy
($\approx$base). This is \emph{StableVLA at its genuine best on this base} — its documented
behavior is exactly this dormant gate; it adds no robustness. AEGIS must beat this ceiling to make
a fair claim, and does. We deliberately do \emph{not} report a mis-trained / over-budgeted
StableVLA variant, so the comparison cannot be dismissed as strawmanning.

\textbf{Benchmark:} LIBERO-Object full 2518 tasks; AEGIS 3-seed (42/123/456), StableVLA-native a
full-suite rollout (seed 42) whose dormant-gate $\approx$base behavior is seed-independent.
Goal/Spatial/Long (even-sampled) run the same 3-arm head-to-head.

\textbf{Object result (3-seed):} AEGIS $\mathbf{51.4\%}$ mean ($52.0/51.0/51.3$; best-of-3 $52.0$)
vs base $45.7\%$ $\Rightarrow$ $\mathbf{+5.7}$ mean / $+6.3$ peak. StableVLA-native $\approx45.7\%$
(dormant gate reproduces base). \textbf{AEGIS improves $\sim+5.7$ over StableVLA's genuine ceiling.}

\textbf{Expected table (Goal/Spatial/Long, to be filled):}
\begin{center}
\small
\begin{tabular}{lcccc}
\toprule
Suite & Base & StableVLA-native & AEGIS & $\Delta$ AEGIS$-$native \\
\midrule
Object  & 45.7 & $\approx$45.7 & \textbf{51.4} & $\sim+5.7$ \\
Goal    & — & — & — & — \\
Spatial & — & — & — & — \\
Long    & — & — & — & — \\
\bottomrule
...
\begin{tabular}{lccc}
\toprule
Arm & Object total & $\Delta$ vs base & note \\
\midrule
VLA-Adapter-Pro (base)        & 45.7 & — & reference \\
StableVLA-native & $\approx$45.7 & $\approx 0$ & dormant gate reproduces base (its ceiling) \\
\textbf{AEGIS} ($\alpha=16$, LoRA+RASF) & \textbf{51.4} & \textbf{+5.7} & 3-seed mean (52.0/51.0/51.3) \\
\bottomrule
\end{tabular}
\end{table}
{\small Head-to-head vs \textbf{StableVLA-native} (its faithful clean-trained config, $\approx$base):
AEGIS improves $+5.7$ mean / $+6.3$ peak over StableVLA's genuine ceiling on LIBERO-Object.
Goal/Spatial/Long run the same 3-arm comparison (in progress).}
...
\begin{enumerate}[nosep]
  ...
  \item \textbf{Beating a fair baseline, not a strawman.} On VLA-Adapter-Pro we compare AEGIS
        head-to-head against \emph{StableVLA-native} — StableVLA in its own faithful clean-trained
        configuration, whose gate goes dormant and reproduces the base ($\approx$base, its
        documented ceiling). AEGIS improves $+5.7$ mean over that ceiling while preserving the
        identity-init guarantee. Comparing against StableVLA's genuine best (not a mis-trained
        variant) is what makes the claim defensible.
\end{enumerate}
