...
\begin{tcolorbox}[colback=pendingcolor!5!white,colframe=pendingcolor!60!black,
  title={\textbf{IN PROGRESS — 4-arm 3-seed eval running (base / SVLA-native / SVLA-budgeted / AEGIS)}},breakable]
...
base architecture used by StableVLA. AEGIS ($\alpha=16$ vision-only LoRA r32 + RASF, $\sim$24.8M)
vs \emph{two} StableVLA configurations, so the comparison cannot be accused of strawmanning:
\begin{itemize}[nosep]
  \item \textbf{StableVLA-native} — the faithful FusedFAN IB-Adapter trained with StableVLA's
        \emph{own} objective (clean-anchor BC; its gate goes dormant $\to$ pass-through). This is
        StableVLA's \emph{documented ceiling} on this base: it adds no robustness, so it reproduces
        the base ($\approx$base). This is the number StableVLA can actually achieve, and the one
        AEGIS must beat to make a fair claim.
  \item \textbf{StableVLA-budgeted} — the same faithful FusedFAN projector + LLM LoRA
        (56.75M trainable, i.e.\ \emph{more} params than AEGIS) \emph{given the same robustness-BC
        training AEGIS receives}, warm-started to a functional start and trained for more steps
        (6{,}500 vs 3{,}000). Under identical robustness training its projector destabilizes the
        closed loop.
\end{itemize}
StableVLA is thus handed the \emph{benefit} on every budget axis (params, steps, warm-start,
faithful arch, trainable gate) and shown at both its best-case (native) and its robustness-trained
form. AEGIS beating the \emph{native} ceiling ($+6.3$ over base) is the load-bearing claim; the
budgeted collapse is the additional architectural finding.

\textbf{Benchmark:} LIBERO-Object suite, full 2518 tasks, 4 arms. Base / StableVLA-budgeted /
AEGIS are 3-seed (42/123/456); StableVLA-native is a single full-suite rollout (seed 42) — its
dormant-gate $\approx$base behavior is seed-independent, so one full-2518 eval establishes the
ceiling without triplicating the cost.

\textbf{Pilot (even-sampled 15/axis, 7 axes, seed 42):}
AEGIS 65/105 (62\%) vs Base 59/105 (56\%) vs StableVLA(budgeted) 21/105 (20\%).

\textbf{Expected table (to be filled — 3-seed, full 2518):}
\begin{center}
\small
\begin{tabular}{lccccc}
\toprule
Axis & Base & SVLA-native & AEGIS & SVLA(bud.) & $\Delta$ AEGIS$-$base \\
\midrule
Background Textures & — & — & — & — & — \\
...
\textbf{Why StableVLA-budgeted collapses (and native does not):} VLA-Adapter closed-loop is
hypersensitive — a $\sim$0.03/step action deviation compounds over chunk execution to full rollout
failure. Robustness-BC drives FusedFAN's projector to deviate ($\Delta_{\text{action}}=0.038$),
collapsing the rollout; the \emph{native} FusedFAN keeps its gate dormant and stays at base.
AEGIS's identity-init + bounded residual holds deviation to $\Delta_{\text{action}}=0.010$ after
$\alpha$-scaling, so it can add robustness without destabilizing — the architectural advantage the
two-config comparison isolates.
\end{tcolorbox}
...
\begin{tabular}{lccc}
\toprule
Arm (full 2518) & Object total & $\Delta$ vs base & note \\
\midrule
VLA-Adapter-Pro (base)        & \pending{} & — & reference \\
StableVLA-native ($\approx$base) & \pending{} & \pending{} & dormant gate, its fair ceiling \\
\textbf{AEGIS} ($\alpha=16$, LoRA+RASF) & \pending{} & \pending{} & \textbf{load-bearing claim} \\
StableVLA-budgeted (robustness-BC) & \pending{} & \pending{} & projector destabilizes \\
\bottomrule
\end{tabular}
\end{table}
{\small Four-arm to be unattackable: AEGIS is compared against StableVLA's \emph{fair ceiling}
(native, $\approx$base) as the load-bearing claim, and additionally against its robustness-trained
form (budgeted). Pilot (s42): AEGIS $62\%$ vs base $56\%$ vs SVLA-budgeted $20\%$.}
